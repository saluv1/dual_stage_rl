...
from src.cil.constraint_provider import constant_constraint_provider
...
class SAC:
    ...
    def _loss_fn_pi(self, policy_params: types.NestedArray, 
        ...
        # rollout:      u_nom -> CIL -> u_safe -> env
        # policy loss:  Q(obs, CIL(u_nom))
        q_actions = self._project_actions_for_q(
            transitions.observations,
            new_actions,
        )

        # We get the predicted new q value using projected action.
        q1_pi = self.apply_q(q1_params, transitions.observations, q_actions)
        q2_pi = self.apply_q(q2_params, transitions.observations, q_actions)
        predicted_new_q_value = jax.lax.min(q1_pi, q2_pi)
        
        # Calculate policy loss
        alpha = getattr(self.config, "alpha", 0.05)
        policy_loss = jnp.mean(alpha * action_log_probs - predicted_new_q_value)
    ...
    def _loss_fn_v(self, v_params: types.NestedArray, 
        ...
        # Calculate predicted q value using projected action.
        q1_pi = self.apply_q(q1_params, transitions.observations, q_actions)
        q2_pi = self.apply_q(q2_params, transitions.observations, q_actions)
        ...
        alpha = getattr(self.config, "alpha", 0.05)
        target_value_func = q_pi - alpha * action_log_probs
    ...
    def _update_fn(self, curr_ls: LearnerState,
        ...
        curr_ls.params.v_target = jax.tree_util.tree_map(lambda x, y: x + self.config.tau * (y - x),
        ...
        logs = {
          ...
          'loss_v': loss_v,
          'entropy': entropy,
          'grad_q1': sum(jnp.sum(jnp.square(p)) for p in jax.tree_util.tree_leaves(grad_q1)),
          'grad_q2': sum(jnp.sum(jnp.square(p)) for p in jax.tree_util.tree_leaves(grad_q2)),
          'grad_pi': sum(jnp.sum(jnp.square(p)) for p in jax.tree_util.tree_leaves(grad_pi)),
          'grad_v': sum(jnp.sum(jnp.square(p)) for p in jax.tree_util.tree_leaves(grad_v)),
        }
    ...
    def _project_actions_for_q(
        ...
        """
        Project batched nominal actions through CIL before evaluating Q.
        """
    ...
    def _get_action(
        ...
        If CIL is enabled:
            policy output -> u_nom -> CIL projection -> u_safe

        Returns:
            u_safe if use_cil=True,
            otherwise vanilla SAC action.
        """
        obs = jnp.reshape(observations, (1, -1))
        mu, sigma = self.apply_policy(policy_params, obs)

        # 1. Nominal SAC action
        if deterministic:
            u_nom_batched = self._transform_action_to_env_spec(mu)
        else:
            u_nom_batched, _ = self._sample_action(key, mu, sigma)

        # Shape: (action_dim,)
        u_nom = jnp.ravel(u_nom_batched)

        # 2. If CIL is off, return nominal SAC action.
        if not self._use_cil:
            return u_nom

        # 3. CIL projection: u_nom -> u_safe
    ...
    def _sample_action(self, key: chex.ArrayNumpy,
        ...
        """
        # Get actual sigma
        sigma = jnp.exp(jnp.clip(sigma, SIGMA_MIN, SIGMA_MAX))
